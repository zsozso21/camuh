...
from fnp.module.heads.multi_sequence_head import MultiSequenceHead


class BertForSequenceClassificationMultiHeadMultiSequence(BertForSequenceClassificationMultiHead):
    def __init__(self, config, *args, **kwargs):
        ...
        self.sequence_num = self.get_params(kwargs, "sequence_num", 1)
        ...
        self.heads = nn.ModuleList([self.head_type(config.hidden_size * (self.sequence_num), self.config.num_labels, self.num_layers,
    ...
    def forward(
        ...
        outputs = self.bert(
            input_ids.view(batch_size * self.sequence_num, max_len),
            attention_mask=attention_mask.view(batch_size * self.sequence_num, max_len),
        ...
        sequence_output = sequence_output.view(input_ids.shape[0], self.sequence_num * self.config.hidden_size)
        if self.selected_layers:
    ...
    def get_head_type(head_type):
        ...
        # if head_type == "multi_sequence":
        #     return BaseSharedHead

        return MultiSequenceHead
